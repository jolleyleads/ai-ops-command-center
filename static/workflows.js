...
async function loadStatus(){try{const r=await fetch('/api/credentials/status');const s=await r.json();const box=document.getElementById('connectorBadges');box.innerHTML=['openai','gmail','twilio'].map(k=>`<span class="badge ${s[k].configured?'ok':'off'}">${k==='twilio'?'SMS':k.toUpperCase()} ${s[k].configured?'●':'○'}</span>`).join('')}catch(e){}}
