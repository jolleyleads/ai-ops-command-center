...
Loaded after outreach_automation so every outreach send uses a durable RFC Message-ID
bound to the external command. Uncertain sends remain blocked until Gmail itself
proves FOUND or, after a grace period, NOT_FOUND. Inconclusive results fail closed.
"""
import base64
import json
import os
...
def _message_identity(command_key: str) -> str:
    ...
    if len(safe) < 16:
        raise ValueError("valid durable command key required for Gmail identity")
...
def _gmail_send_identified(to_email: str, subject: str, body: str, *, command_key: str, thread_id: str="") -> Dict[str,Any]:
    """Send once with a provider-searchable deterministic RFC Message-ID."""
    try:
        token=gmail_access_token()
        identity=_message_identity(command_key)
        msg=EmailMessage()
        msg["To"]=to_email
        msg["Subject"]=subject
        msg["Message-ID"]=identity
        msg["X-AI-Ops-Command-ID"]=command_key
        ...
        if not response.ok:
            return {"ok":False,"error":f"Gmail error {response.status_code}: {response.text[:500]}","message_identity":identity}
        data=response.json()
        return {"ok":True,"message_id":data.get("id"),"thread_id":data.get("threadId") or thread_id,"message_identity":identity,"raw":data}
    except Exception as exc:
        return {"ok":False,"error":oa._clean(exc,1000),"message_identity":_message_identity(command_key)}
...
def gmail_reconciliation_proof(cmd) -> Dict[str,Any]:
    """Return found / not_found / inconclusive using Gmail's own Sent mailbox."""
    if not cmd or cmd.kind!="gmail_send":
        return {"ok":False,"outcome":"inconclusive","error":"GMAIL_SEND_COMMAND_REQUIRED"}
    try:
        payload=json.loads(cmd.payload_json or "{}")
    except Exception:
        return {"ok":False,"outcome":"inconclusive","error":"INVALID_COMMAND_PAYLOAD"}
    recipient=normalize_email(payload.get("recipient"))
    subject=str(payload.get("subject") or "")
    identity=_message_identity(cmd.idempotency_key)
    try:
        token=gmail_access_token();headers={"Authorization":f"Bearer {token}"}
        q=f'in:sent rfc822msgid:{identity}'
        ...
        if not response.ok:
            return {"ok":False,"outcome":"inconclusive","source":"gmail_messages_list","query":q,"http_status":response.status_code,"error":response.text[:500]}
        rows=response.json().get("messages") or []
        if len(rows)>1:
            return {"ok":False,"outcome":"inconclusive","source":"gmail_messages_list","query":q,"match_count":len(rows),"error":"MULTIPLE_IDENTITY_MATCHES"}
        if len(rows)==1:
            message_id=str(rows[0].get("id") or "")
            detail=requests.get(f"https://gmail.googleapis.com/gmail/v1/users/me/messages/{message_id}",headers=headers,params={"format":"metadata","metadataHeaders":["Message-ID","To","Subject","X-AI-Ops-Command-ID"]},timeout=30)
            if not detail.ok:
                return {"ok":False,"outcome":"inconclusive","source":"gmail_message_get","message_id":message_id,"http_status":detail.status_code,"error":detail.text[:500]}
            data=detail.json();h=_headers(data)
            exact_identity=h.get("message-id","").strip().lower()==identity.lower()
            exact_recipient=normalize_email(parseaddr(h.get("to","") )[1])==recipient
            exact_command=h.get("x-ai-ops-command-id","").strip()==cmd.idempotency_key
            if not (exact_identity and exact_recipient and exact_command):
                return {"ok":False,"outcome":"inconclusive","source":"gmail_message_get","message_id":message_id,"identity_match":exact_identity,"recipient_match":exact_recipient,"command_match":exact_command,"error":"GMAIL_IDENTITY_MISMATCH"}
...
def reconcile_gmail_command(cmd) -> Dict[str,Any]:
    ...
    proof=gmail_reconciliation_proof(cmd)
    outcome=proof.get("outcome")
    if outcome=="inconclusive":
        oa._audit_actor(cmd.lead_id,"gmail_reconciliation_inconclusive",{"command_key":cmd.idempotency_key},{"status":"uncertain","proof":proof},"system")
        db.session.commit()
        return {"ok":False,"status":"uncertain","proof":proof}
    result=oa._reconcile_external_command(cmd,outcome=="found",proof)
    attempt=oa.OutreachSendAttempt.query.filter_by(idempotency_key=json.loads(cmd.payload_json or "{}").get("send_key","")).first()
    lead=db.session.get(oa.OutreachLead,cmd.lead_id)
    if outcome=="found":
        if attempt:
            attempt.status="sent";attempt.message_id=proof.get("message_id","");attempt.thread_id=proof.get("thread_id","");attempt.error="";attempt.updated_at=datetime.utcnow()
        if lead:
            lead.status="sent";lead.gmail_message_id=proof.get("message_id","");lead.gmail_thread_id=proof.get("thread_id","");lead.sent_at=lead.sent_at or datetime.utcnow();lead.last_error=""
    else:
        if attempt:
            attempt.status="failed";attempt.error="RECONCILIATION_PROVED_NO_SIDE_EFFECT";attempt.updated_at=datetime.utcnow()
        if lead: lead.last_error="RECONCILIATION_PROVED_NO_SIDE_EFFECT"
    oa._audit_actor(cmd.lead_id,"gmail_reconciliation_applied",{"command_key":cmd.idempotency_key},{"outcome":outcome,"command_status":result.get("status"),"proof":proof},"system")
    db.session.commit()
    return {"ok":True,"outcome":outcome,"command_status":result.get("status"),"proof":proof}


def hardened_safe_send(lead, *, kind: str, sequence: int, subject: str, body: str) -> Dict[str,Any]:
    ...
    attempt=oa.OutreachSendAttempt.query.filter_by(idempotency_key=key).first();ag=send_attempt_gate(attempt.status if attempt else "")
    ...
    if attempt is None:
        attempt=oa.OutreachSendAttempt(lead_id=lead.id,kind=kind,sequence=sequence,recipient=address,idempotency_key=key,status="pending");db.session.add(attempt)
        existing=oa._commit_unique_or_existing(oa.OutreachSendAttempt,{"idempotency_key":key})
    ...
    payload={**oa._serialize(lead),"contact_email":address,"subject":subject,"body":body}
    command_payload={"recipient":address,"kind":kind,"sequence":sequence,"subject":subject,"body":body,"send_key":key}
    cmd=oa._enqueue_external_command(lead,"gmail_send",command_payload)
...
def operator_gmail_reconcile(command_id:int):
    ...
    result=reconcile_gmail_command(cmd)
    return jsonify(result),200 if result.get("ok") else 409


@app.route("/api/operator/gmail-recovery/<int:command_id>/retry",methods=["POST"])
def operator_gmail_retry(command_id:int):
    if not oa._operator_session_authorized(): return jsonify({"ok":False,"error":"operator session required"}),401
    if not oa._csrf_ok(): return jsonify({"ok":False,"error":"CSRF validation failed"}),403
    cmd=db.session.get(oa.ExternalSideEffectCommand,command_id)
    if not cmd or cmd.kind!="gmail_send" or cmd.status!="failed" or cmd.error!="RECONCILIATION_PROVED_NO_SIDE_EFFECT":
        return jsonify({"ok":False,"error":"negative Gmail reconciliation proof required"}),409
    payload=json.loads(cmd.payload_json or "{}")
    attempt=oa.OutreachSendAttempt.query.filter_by(idempotency_key=payload.get("send_key","")).first();lead=db.session.get(oa.OutreachLead,cmd.lead_id)
    result=retry_after_negative_reconciliation(lead,attempt)
    oa._audit_actor(cmd.lead_id,"gmail_reconciled_retry",{"predecessor_command":cmd.id,"predecessor_key":cmd.idempotency_key},{"ok":result.get("ok") is True,"result":result},"operator")
    db.session.commit()
    return jsonify(result),200 if result.get("ok") else 409
