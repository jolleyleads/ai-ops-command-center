import json
import os
import re

from openai import OpenAI


def _json_object(text):
    """Parse model JSON without crashing the whole research request on truncated prose/JSON."""
    text = (text or "").strip()
    if text.startswith("```"):
        text = text.split("\n", 1)[-1].rsplit("```", 1)[0].strip()
    try:
        value = json.loads(text)
        return value if isinstance(value, dict) else {}
    except (json.JSONDecodeError, TypeError):
        # Recover the first complete JSON object if the model wrapped it in text.
        start = text.find("{")
        if start >= 0:
            decoder = json.JSONDecoder()
            try:
                value, _ = decoder.raw_decode(text[start:])
                return value if isinstance(value, dict) else {}
            except json.JSONDecodeError:
                pass
        return {}


def _client():
    return OpenAI(api_key=os.environ["OPENAI_API_KEY"], timeout=7.0, max_retries=0)


def _fallback_plan(query, location):
    base = " ".join(x for x in (query.strip(), location.strip()) if x)
    return {
        "goal": query,
        "intent": "web_research",
        "concepts": [],
        "queries": [base or query],
        "verification_criteria": ["source-backed evidence relevant to the user's inquiry"],
        "sufficient": False,
        "gaps": [],
    }


def plan_research(query, location="", prior_evidence=None):
    if not os.getenv("OPENAI_API_KEY"):
        return None
    evidence = (prior_evidence or [])[-8:]
    prompt = {"query": query, "location": location, "prior_evidence": evidence}
    instructions = """Plan one fast web-research pass for an arbitrary user inquiry. Return ONLY one compact valid JSON object with keys: goal, intent, concepts, queries, verification_criteria, sufficient, gaps. queries must contain 1-3 concise targeted web searches. Do not hard-code an industry or location; preserve the supplied location when relevant. Use semantic alternatives. Never invent evidence. Keep the entire JSON response short enough to finish."""
    response = _client().responses.create(
        model=os.getenv("OPENAI_MODEL", "gpt-5.6-luna"),
        instructions=instructions,
        input=json.dumps(prompt, ensure_ascii=False),
        max_output_tokens=700,
    )
    parsed = _json_object(response.output_text)
    if not parsed.get("queries"):
        return _fallback_plan(query, location)
    return parsed


def evaluate_research(query, location, evidence):
    if not os.getenv("OPENAI_API_KEY"):
        return None
    compact=[]
    for item in evidence[:10]:
        compact.append({"title":str(item.get("title") or "")[:250],"url":str(item.get("url") or "")[:800],"text":str(item.get("page_text") or item.get("subtitle") or "")[:1000]})
    instructions = """Evaluate supplied research evidence quickly. Return ONLY one compact valid JSON object with keys: sufficient, answer_summary, gaps, followup_queries, ranked_urls. Never invent evidence. followup_queries may contain at most 2 concise searches. Direct page text is stronger than snippets. Keep the response short enough to finish."""
    response = _client().responses.create(
        model=os.getenv("OPENAI_MODEL", "gpt-5.6-luna"),
        instructions=instructions,
        input=json.dumps({"query":query,"location":location,"evidence":compact},ensure_ascii=False),
        max_output_tokens=700,
    )
    parsed = _json_object(response.output_text)
    return parsed if parsed else {"sufficient":False,"answer_summary":"","gaps":[],"followup_queries":[],"ranked_urls":[]}
