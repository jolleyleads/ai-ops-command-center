...
def _next_available_slot(attendee_email: str):
    ...
    for day_offset in range(7):
        ...
        for hour in (10, 11, 13, 14, 15, 16):
            ...
            checks.append({"start": req["start"], "ok": result.get("ok"), "available": result.get("available")})
...
def production_e2e():
    ...
    # 1. Create a test lead that still passes the exact production qualification gate.
    source_url = "https://mail.google.com/"
    evidence = [{
        "url": source_url,
        "email": recipient,
        "title": "Controlled production E2E test contact",
        "snippet": "Operator-controlled Gmail recipient for production integration verification.",
        "observed_at": datetime.utcnow().isoformat() + "Z",
    }]
    lead = oa.OutreachLead(
        company=f"AI Ops Production E2E {run_id}",
        contact_email=recipient,
        contact_name="Production E2E",
        location="Portsmouth, VA",
        source_url=source_url,
        evidence_json=oa._canonical_json(evidence),
        score=100,
        verification="controlled_e2e",
    ...
    # 2. Real Gmail send through the same guarded production send path.
    ...
    lead.subject, lead.body = subject, body
    db.session.commit()
    ...
    lead.gmail_message_id, lead.gmail_thread_id = message_id, thread_id
    lead.sent_at = datetime.utcnow()
    lead.status = "sent"
    db.session.commit()

    # 3. Deterministic interested reply simulation. No inbound email is fabricated;
    # this verifies the reply->booking decision path using explicit test text.
    ...
    # 4. Check real Calendar availability and choose the first available controlled slot.
    ...
    # 5. Create through the same production reply-booking handoff and side-effect outbox.
    booking_result = oa.process_reply_to_booking(
        ...
        event_create_func=lambda req: oa._calendar_create_via_command(
            lead, req,
            summary=f"AI Ops Production E2E {run_id}",
            description="Controlled production E2E test event.",
            idempotency_key=f"aoce2e{lead.id}booking",
        ),
    )
    ...
    # Provider shapes can evolve; also inspect the persisted booking command receipt.
    ...
    report["steps"]["calendar_create"] = {"pass": create_pass, "stage": booking_result.get("stage"), "event_id": event_id, "event_url": provider.get("event_url") or command_receipt.get("event_url") or ""}
    if not create_pass:
        report["pass"] = False
        return jsonify(report), 502

    # 6. Read the event back from Google. Creation without durable read-back is a FAIL.
    proof = get_event(event_id)
    durable_pass = proof.get("ok") is True and proof.get("found") is True
    report["steps"]["calendar_durable_verification"] = {"pass": durable_pass, "event_id": event_id, "found": proof.get("found"), "start": proof.get("start"), "end": proof.get("end"), "event_url": proof.get("event_url")}
    report["pass"] = bool(gmail_pass and durable_pass)
    report["ok"] = report["pass"]
    ...
    db.session.commit()
    return jsonify(report), 200 if report["pass"] else 502
