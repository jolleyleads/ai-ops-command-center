...
def _search(q,loc):
    """Search independent providers, not just the legacy public-record router."""
    results=[]; sources=[]
    def add(payload):
        if not isinstance(payload,dict): return
        results.extend(payload.get("results") or [])
        s=_clean(payload.get("source"),200)
        if s and s not in sources:sources.append(s)
    try:add(_search_public_records(q,loc))
    except Exception:app.logger.exception("RESEARCH_PUBLIC_SEARCH_ERROR query=%r",q)
    try:add(contractor_intent._cross_checked_evidence_search(q,loc))
    except Exception:app.logger.exception("RESEARCH_MULTI_PROVIDER_ERROR query=%r",q)
...
def _page(url):
    try:
        ...
        r=requests.get(url,timeout=3,headers={"User-Agent":"Mozilla/5.0 AI-Ops-Research-Agent/5.1"},allow_redirects=True)
...
def _infer_location(q,loc,plan):
    ...
    for key in ("location","place","region"):
        ...
        if v:return v
    patterns=[r"\b(?:in|near|around)\s+([A-Za-z .'-]+?,?\s*(?:VA|Virginia|NC|Maryland|MD|DC))\b",r"\b(Richmond|Norfolk|Portsmouth|Chesapeake|Suffolk|Virginia Beach|Hampton|Newport News)\s*,?\s*(VA|Virginia)?\b"]
    for pat in patterns:
        m=re.search(pat,q,re.I)
        if m:return _clean(" ".join(x for x in m.groups() if x) if len(m.groups())>1 else m.group(1),200)
    return ""
def _tools(plan,q,loc,started):
    ...
    try:
        business_query="electrical contractors" if any(x in text for x in ("electrical","electrician","permit")) else q
        if any(x in text for x in ("business","contractor","company","local","provider","permit","electric")):
            out.extend((_search_businesses(business_query,loc).get("results") or [])[:12])
        if any(x in text for x in ("contractor","permit","electric")) and time.monotonic()-started<35:
            out.extend((contractor_intent._contractor_intent_search(business_query,loc).get("results") or [])[:12])
    except Exception:app.logger.exception("RESEARCH_TOOL_ERROR intent=%r",intent)
    return out

def _candidate_names(items):
    bad=("indeed","linkedin","ziprecruiter","glassdoor","permit","jobs","hiring","search results","city of","richmond")
    out=[]
    for x in items:
...
def _candidate_verify(names,q,loc):
    queries=[]
    for n in names[:6]:queries.extend([f'"{n}" {loc} "pull permits"',f'"{n}" {loc} permit permitting contractor',f'"{n}" {loc} hiring "master electrician" "qualifying agent"'])
    return _batch(queries[:12],loc) if queries else ([],[])
...
def _expansion_queries(q,plan,tried,loc):
    intent=_clean(plan.get("intent"),200).lower(); goal=_clean(plan.get("goal") or q,500); concepts=" ".join(_clean(x,100) for x in (plan.get("concepts") or []) if _clean(x,100)); subject="electrical contractors" if any(x in (q+intent).lower() for x in ("electrical","electrician")) else "contractors companies"
    candidates=[goal,f'{subject} {loc}',f'{subject} {loc} permits',f'{subject} {loc} hiring',f'{loc} permit records contractor applicant electrical',f'{loc} electrical permit applications contractor',f'{loc} electrician "pull permits"',f'{loc} "master electrician" "pull permits"',f'{loc} "qualifying agent" electrical contractor']
...
def _smart_search(q,loc):
    ...
    try:
        ...
        queries=queries[:4] or [direct or q]; live,src=_batch(queries,loc); tried.extend(queries); sources.extend(src); evidence=_dedupe(live+memory)
        if time.monotonic()-started<18:_inspect(evidence,8)
        evaluation=evaluate_research(q,loc,evidence) if time.monotonic()-started<22 else {}; evaluation=evaluation or {}; weak=(not evidence) or (not evaluation.get("sufficient")) or len(evidence)<3
        if weak and time.monotonic()-started<28:
            ...
            if expansion:
                ...
                if time.monotonic()-started<34:_inspect(evidence,10)
        names=_candidate_names(evidence)
        if names and time.monotonic()-started<37:
            ...
            if time.monotonic()-started<42:_inspect(verified,8)
...
def test_smart_search():
    q=_clean(request.args.get("query") or "machine learning engineer jobs",500); loc=_clean(request.args.get("location") or "",200); p=_smart_search(q,loc); app.logger.warning("SMART_SEARCH_DIAGNOSTIC agent=%r multi=%r rag=%r expanded=%r candidate_verification=%r framework=%r intent=%r count=%s promoted=%s runtime_ms=%s query=%r",p.get("agent_mode"),p.get("multi_provider"),p.get("rag_enabled"),p.get("search_expanded"),p.get("candidate_verification"),p.get("framework"),p.get("intent"),p.get("count",0),p.get("promoted_count",0),p.get("runtime_ms"),q); return jsonify(p)
