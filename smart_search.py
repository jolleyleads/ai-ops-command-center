...
def _page(url):
    try:
        ...
        r=requests.get(url,timeout=2.5,headers={"User-Agent":"Mozilla/5.0 AI-Ops-Research-Agent/3.0"},allow_redirects=True)
...
def _tools(plan,q,loc,started):
    if time.monotonic()-started>18:return []
    ...
    try:
        ...
        if "contractor" in intent and time.monotonic()-started<21:
...
def _smart_search(q,loc):
    ...
    try:
        ...
        # Persist only retrieved/crawled source material before the LLM can add any prose.
        ...
        if follow and time.monotonic()-started<21:
            more,src2=_batch(follow,loc); tried.extend(follow); evidence=_dedupe(evidence+more); sources.extend(x for x in src2 if x not in sources)
...
def test_smart_search():
    ...
    app.logger.warning("SMART_SEARCH_DIAGNOSTIC agent=%r rag=%r framework=%r intent=%r count=%s promoted=%s runtime_ms=%s query=%r",p.get("agent_mode"),p.get("rag_enabled"),p.get("framework"),p.get("intent"),p.get("count",0),p.get("promoted_count",0),p.get("runtime_ms"),q); return jsonify(p)
