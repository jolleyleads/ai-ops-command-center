...
def openai_text(prompt, instructions="", model=None):
    ...
    if not api_key:
        raise RuntimeError("OPENAI_API_KEY is not configured on Render")
    ...
    try:
        ...
        raise RuntimeError(
            "OpenAI workflow request timed out after 55 seconds. "
            "The workflow was stopped cleanly instead of hanging."
        )

    except requests.RequestException as e:
        raise RuntimeError(
            f"OpenAI connection error: {str(e)[:300]}"
        )

    if not r.ok:
        raise RuntimeError(
            f"OpenAI error {r.status_code}: {r.text[:600]}"
        )
...
def gmail_access_token():
    ...
    if not all([refresh, client_id, client_secret]):
        raise RuntimeError(
            "Gmail is not configured. Add GMAIL_ACCESS_TOKEN or "
            "GOOGLE_CLIENT_ID + GOOGLE_CLIENT_SECRET + GOOGLE_REFRESH_TOKEN on Render"
        )
    ...
    if not r.ok:
        raise RuntimeError(
            f"Google token error {r.status_code}: {r.text[:500]}"
        )
...
def send_gmail(to, subject, body, from_email=None):
    ...
    if not r.ok:
        raise RuntimeError(
            f"Gmail error {r.status_code}: {r.text[:600]}"
        )
...
def send_sms(to, body, from_number=None):
    ...
    if not all([sid, token, from_number]):
        raise RuntimeError(
            "Twilio is not configured on Render"
        )
    ...
    if not r.ok:
        raise RuntimeError(
            f"Twilio error {r.status_code}: {r.text[:600]}"
        )
...
def execute_node(node, context):
    ...
    if node_type == "condition":
        ...
        return {
            ...
            "branch": branch,
            "context": context,
            "summary": f"Condition = {result}; branch = {branch.upper()}"
        }
    ...
    if node_type == "ai":
        ...
        instructions = render_template_string(
            config.get("instructions", ""),
            context
        )

        text, response_id = openai_text(
            prompt,
            instructions,
        ...
        context[key] = text
        context["openai_response_id"] = response_id

        return {
            "ok": True,
            "context": context,
            "summary": f"OpenAI wrote {len(text)} chars"
        }
    ...
    if node_type == "http":
        ...
        if isinstance(body, dict):
            body = {
                k: render_template_string(v, context)
    ...
    if node_type == "email":
        ...
        if not to:
            return {
                "ok": False,
                "error": "Gmail node requires a recipient"
            }
    ...
    if node_type == "sms":
        ...
        if not to or not body:
            return {
                "ok": False,
                "error": "SMS node requires To and Body"
            }
...
def run_workflow(workflow, payload):
    ...
    nodes = definition.get(
        "nodes",
        []
    )

    edges = definition.get(
        "edges",
        []
    )
    ...
    while queue:
        ...
        logs.append({
            "node_id": nid,
            "node": node.get("label") or node.get("type"),
            "type": node.get("type"),
            "result": result
        })
...
def index():
    ...
    jobs = SavedJob.query.order_by(
        SavedJob.created_at.desc()
    ).limit(20).all()

    workflow_count = Workflow.query.count()

    leads = QualifiedLead.query.order_by(
...
def workflow_detail(workflow_id):
    ...
    if request.method == "GET":
        return jsonify({
            ...
            "definition": parse_definition(wf)
        })
    ...
    wf.schedule = data.get(
        "schedule",
        wf.schedule
    )

    if "definition" in data:
        wf.definition = json.dumps(
            data["definition"]
        )
...
def run_workflow_api(workflow_id):
    ...
    if (
        ok
        and output.get("condition_branch") == "true"
        and output.get("lead")
    ):
        try:
            existing = QualifiedLead.query.filter_by(
            ...
            if not existing:
                saved_lead = QualifiedLead(
                    ...
                    source=(
                        wf.name
                        or "Electrical Contractor Lead Qualification"
                    )
                )
...
@app.route("/api/runs")
def runs_api():
    rows = WorkflowRun.query.order_by(
...
def credentials_api():
    if request.method == "GET":
        rows = Credential.query.order_by(
...
@app.route(
    "/api/qualified-leads/<int:lead_id>/pipeline",
    methods=["GET", "POST"]
)
def qualified_lead_pipeline_api(lead_id):
    lead = QualifiedLead.query.get_or_404(
        lead_id
    )

    row = LeadPipeline.query.filter_by(
    ...
    return jsonify([
        {
            ...
            "stage": (
                pipeline_by_lead[lead.id].stage
                if lead.id in pipeline_by_lead
                else "New"
            ),
            "notes": (
                pipeline_by_lead[lead.id].notes
                if lead.id in pipeline_by_lead
                else ""
            ),
            "follow_up_date": (
                pipeline_by_lead[lead.id].follow_up_date
                if lead.id in pipeline_by_lead
                else ""
            )
        }
...
def qualified_leads_api():
    rows = QualifiedLead.query.order_by(
        QualifiedLead.created_at.desc()
    ).limit(100).all()
