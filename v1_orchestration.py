...
from outreach_bridge import (
    ...
    _clean,
    _contractor_search,
    _evidence_for_storage,
    _evidence_score,
    _verified,
    _verified_public_email,
)
from outreach_automation import (
    FIRST_FOLLOWUP_DAYS,
    OutreachLead,
    _draft_email,
    _gmail_thread_reply_state,
    _persist_reply_evidence,
    _route_persisted_reply,
    _safe_send,
    _store_qualification,
)
...
def _evidence_with_contact(result, email, email_source):
    evidence = [x for x in _evidence_for_storage(result) if isinstance(x, dict)]
    if email and email_source:
        evidence.append({"email": email, "url": email_source, "source": "verified_public_contact"})
    return evidence
...
def orchestrate_discovery(payload):
    ...
    if not _contractor_search(payload):
        ...
        summary["paused_reason"] = "not_contractor_or_permit_search"
        return summary
    ...
    for result in payload.get("results") or []:
        ...
        if not company or score < QUEUE_MIN_SCORE or not _verified(result):
            continue
        summary["eligible"] += 1
        urls = _candidate_urls(result)
        source_url = urls[0] if urls else ""
        if not source_url:
            summary["skipped"].append({"company": company, "reason": "missing_source_url"})
            continue
        existing = OutreachLead.query.filter_by(company=company, source_url=source_url).first()
        if existing:
            summary["skipped"].append({"company": company, "reason": "already_queued", "lead_id": existing.id})
            continue
        ...
        lead = OutreachLead(
            company=company,
            contact_email=email,
            contact_name=_clean(result.get("contact_name"), 300),
            location=_clean(result.get("location") or payload.get("location"), 300),
            source_url=source_url,
            evidence_json=json.dumps(evidence),
            score=score,
            verification=_clean(result.get("verification"), 100) or "SOURCE_VERIFIED",
        ...
        if not qualification.get("ok"):
            summary["skipped"].append({"company": company, "lead_id": lead.id, "reason": "qualification_failed", "reason_codes": qualification.get("reason_codes") or []})
            continue
        summary["qualified"] += 1
        if not email:
            summary["skipped"].append({"company": company, "lead_id": lead.id, "reason": "no_verified_public_email"})
            continue
        ...
        if not drafted.get("ok"):
            ...
            db.session.commit()
            summary["skipped"].append({"company": company, "lead_id": lead.id, "reason": "draft_failed"})
            continue
        ...
        if not AUTOSEND_ENABLED:
            summary["skipped"].append({"company": company, "lead_id": lead.id, "reason": "autosend_disabled", "status": "drafted"})
            continue

        execution = _safe_send(lead, kind="initial", sequence=0, subject=lead.subject, body=lead.body)
        if not execution.get("ok"):
            lead.last_error = _clean(execution, 2000)
            db.session.commit()
            summary["skipped"].append({"company": company, "lead_id": lead.id, "reason": "safe_send_failed"})
            continue
        receipt = execution.get("send_receipt") or {}
        now = datetime.utcnow()
        lead.gmail_message_id = _clean(receipt.get("message_id"), 255)
        lead.gmail_thread_id = _clean(receipt.get("thread_id"), 255)
        lead.sent_at = now
        lead.follow_up_due_at = now + timedelta(days=FIRST_FOLLOWUP_DAYS)
        lead.status = "sent"
        lead.last_error = ""
        lead.updated_at = now
        db.session.commit()
...
def scan_real_inbound_replies():
    ...
    ).order_by(OutreachLead.id.asc()).all()
    processed = []
    for lead in leads:
        reply = _gmail_thread_reply_state(lead.gmail_thread_id)
        if not reply.get("ok"):
            processed.append({"lead_id": lead.id, "ok": False, "stage": "reply_check_failed", "reason": reply.get("reason"), "error": reply.get("error")})
            continue
        if not reply.get("replied"):
            processed.append({"lead_id": lead.id, "ok": True, "stage": "no_reply"})
            continue
...
def v1_discovery_orchestration_hook(response):
    ...
    try:
        ...
        payload["outreach_automation"] = orchestrate_discovery(payload)
        response.set_data(app.json.dumps(payload))
        response.headers["Content-Type"] = "application/json"
        response.headers["Content-Length"] = str(len(response.get_data()))
    except Exception as exc:
        app.logger.exception("V1_DISCOVERY_ORCHESTRATION_ERROR %s", type(exc).__name__)
    return response
